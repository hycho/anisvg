		...
		try {
			...
			if (value.length() == 13) {
				...
				if (_j_month <= 0 || _j_month > 12) {
					...
					_check_i = (_j_11 * 2) + (_j_12 * 3) + (_j_13 * 4) + (_j_14 * 5) + (_j_15 * 6) + (_j_16 * 7) + (_j_21 * 8) + (_j_22 * 9) + (_j_23 * 2)
							+ (_j_24 * 3) + (_j_25 * 4) + (_j_26 * 5);
		...
		try {
			...
			if (checkpattern && _return_s.equals("")) {
				...
				while (_matcher.find()) {
					...
					if (_inner_debugging_b)
						System.out.println("getting id : " + _s);
...
/**
	...
	public static String checkNoneAllowChar(String value, char[] nonallowchar) {
		...
		if (value.trim().length() > 0) {
			...
			outer : for (int i = 0; i < _array.length; i++) {
	...
	/**
	 * @return boolean 입력값이 null 이 아니고 "true", "yes", "t", "y"(대소문자 구분 없음, trim
	 *         함)이면 true 반환
	 * @param value
	 *            입력값
	 */
	public static boolean isTrue(Object obj) {
		...
		if (!isNull(obj)) {
			if (obj instanceof String) {
				if (((String) obj).trim().equalsIgnoreCase("true") || ((String) obj).trim().equalsIgnoreCase("yes")
						|| ((String) obj).trim().equalsIgnoreCase("t") || ((String) obj).trim().equalsIgnoreCase("y")) {
					_return = true;
				}
			}
		}
