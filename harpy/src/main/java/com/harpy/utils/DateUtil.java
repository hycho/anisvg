...
public class DateUtil {
	 ...
	 *            가감할 일수
	 * @param jumpdays
	 *            인수 배열 길이 7, 일~토요일 순, 예)new boolean[]{true, false, false,
	 *            false, false, false, true}
	 * @param isapplystartdate
	 *            시작일 부터 jumpdays 사항을 적용할지 여부, 아주 특이한 케이스에 한해 false 로 할것을 권고
	...
	public static Date addDays(Date date, int terms, boolean[] jumpdays, boolean isapplystartdate) throws Exception {
		...
		if (terms != 0) {
			...
			for (int i = 0; i < (terms * _term);) {
				...
				if (jumpdays == null) {
					i++;
				} else if (!((_calendar.get(Calendar.DAY_OF_WEEK) == SUNDAY && jumpdays[0]) || (_calendar.get(Calendar.DAY_OF_WEEK) == MONDAY && jumpdays[1])
						|| (_calendar.get(Calendar.DAY_OF_WEEK) == TUESDAY && jumpdays[2]) || (_calendar.get(Calendar.DAY_OF_WEEK) == WEDNESDAY && jumpdays[3])
						|| (_calendar.get(Calendar.DAY_OF_WEEK) == THURSDAY && jumpdays[4]) || (_calendar.get(Calendar.DAY_OF_WEEK) == FRIDAY && jumpdays[5]) || (_calendar
						.get(Calendar.DAY_OF_WEEK) == SATURDAY && jumpdays[6]))) {
					i++;
				}
	 ...
	 * @see #addDays(Date, int, boolean[], boolean)
	 * @param value
	 *            포맷에 맞춘 기준 일시 값, as like : 20101130, 20101130120101
	 * @param format
	 *            기준 일시에 대한 포맷, as like : yyyyMMdd, yyyyMMddHHmmss
	 * @throws Exception
	 */
	 ...
	 *            Date 기준 일시
	 * @param type
	 *            가감할 타입, Calendar.YEAR / Calendar.MONTH / Calendar.HOUR /
	 *            Calendar.MINUTE
	 * @param terms
	 *            가감할 수
	 * @return Date 계산된 일시
	 ...
	 *            기준 일시
	 * @param format
	 *            일시 포맷, as like : yyyyMMdd, yyyy/MM/dd, yyyy년 MM월 dd일, HHmmss,
	 *            HH:mm:ss, HH : mm, ...
	 ...
	 *            기준 일시
	 * @param dayofweek
	 *            요일, 일(1)~토(7), 벗어난 값에 대해서는 그 수치에 해당하는 만큼 과거 미래에 해당하는 일시를 반환하도록
	 *            한다.
	 ...
	 * 
	 * @param starttime
	 *            시작일시에 대한 millisecond
	 * @param endiime
	 *            종료일시에 대한 millisecond
	...
	public static long getWaitingTime(String start_hhmmss) {
		...
		try {
			...
			if (Long.parseLong((new SimpleDateFormat("HHmmss")).format(_now).toString()) > Long.parseLong(start_hhmmss)) {
				...
				// '24시 - (현재 시간 - 배치 프로그램 시간)' 으로 배치 프로그램이 수행 되기 까지 기다릴 시간을
				// 얻는다.
	 ...
	 *            기준 일시
	 * @param dayofweek
	 *            일(1)~토(7), 범위를 벋어나는 값에 대해서도 오류 없이 결과를 반환한다.
