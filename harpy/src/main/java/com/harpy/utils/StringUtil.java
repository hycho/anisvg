...
public final class StringUtil {
			...
			if (!isBlank(value)) {
				if (isBlank(format)) {
					...
					if (value.length() > _check_format.length()) {
						// value 값이 format 보다 크다면 value 길이에 맞춰 format 뒤에 's' 를
						// 붙인다.
	 ...
	 * @param value
	 *            입력값이 null, ""(trim 적용 여부는 인수에 의해 적용) 이면 true 반환
	 * @param istrim
	 *            입력값을 trim 할것인지 여부
	 * @return
	 */
	 ...
	 *            리턴될 문자열 전체 길이
	 * @param lengthbybyte
	 *            기준문자 와 캐릭터의 길이를 byte 로 체크할것인지 여부
	 * @param paddingchar
	 *            주어진 길이를 채우는데 사용할 캐릭터
	 * @param toright
	 *            기준 문자의 오른쪽에 채울지 여부, false 일 경우 문자열 앞(왼쪽)에 붙인다.
	 * @return 가공된 문자열
	 */
