...
public class MD5Util {
	...
	private static byte padding[] = {(byte) 0x80, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0,
	...
	private final void decode(int output[], byte input[], int off, int len) {
		...
		int j = 0;
		for (; j < len; i++, j += 4) {
			output[i] = (((int) (input[off + j] & 0xff)) | (((int) (input[off + j + 1] & 0xff)) << 8) | (((int) (input[off + j + 2] & 0xff)) << 16) | (((int) (input[off
					+ j + 3] & 0xff)) << 24));
		}
	}
	...
	public String getMD5String() throws Exception {
		byte b[] = getDigest();
		return stringify(b);
	}
